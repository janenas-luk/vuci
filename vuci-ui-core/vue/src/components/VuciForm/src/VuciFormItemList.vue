<template>
  <vuci-form-item-template v-bind="VuciFormItemTemplateProps" ref="form-item" :rules="listRules">
    <div class="vuci-form-list">
...
export default {
  ...
  methods: {
    ...
    validate () {
      ...
      this.$nextTick(() => this.vuciForm.validateField(this.prop, errMsg => {
        this.error = errMsg
        this.model.splice(-1)
      }))
    },
    ...
    handleinputConfirm () {
      if (this.error) {
        this.$nextTick(() => this.vuciForm.validateField(this.prop))
        this.error = null
