...
export default {
  ...
  props: {
    ...
    */
    save: Function,
    /*
    ** If this function is provided, it will be called when vuci applys the uci configuration.
    ** Parameters: self
    ** Return:  Promise object or undefined
    */
    apply: Function,
    rules: {
      validator (value) {
        if (typeof value === 'string') return Object.keys(VuciValidator).indexOf(value) !== -1

        if (typeof value === 'object') {
          return Object.keys(VuciValidator).indexOf(value.type) !== -1
        }
  ...
  computed: {
    ...
    config () {
      return this.vuciSection.config
    },
    form () {
      ...
      if (typeof this.rules === 'string') {
        const rule = {
          validator: (rule, value) => {
            ...
            return VuciValidator[this.rules](value)
          },
        ...
        rules.push({
          validator: (rule, value) => {
            if (!value) return true
            return VuciValidator[this.rules.type](value)
          },
    ...
    visible () {
      ...
      for (const i in depend.names) {
        ...
        const f = this.vuciForm.fields[`${this.sid}_${name}`]
  ...
  methods: {
    ...
    notifyChanged () {
      ...
      this.vuciSection.$emit(`change-${this.name}`, this)
      this.vuciSection.$emit('change', this.sid, this.vuciSection)
    },
    watchValue () {
  ...
  beforeDestroy () {
    this.$delete(this.vuciForm.fields, this.prop)
  }
}
</script>
