  ...
  <vuci-form uci-config="wireless">
    <a-tabs v-if="radios.length > 0" name="top" :animated="false">
      <a-tab-pane v-for="radio in radios" :key="radio.name" :tab="radio.name + ` (${radio.hardware})`">
        <vuci-named-section :name="radio.name" v-slot="{ s }" :card="false">
          <a-tabs>
            <a-tab-pane :tab="$t('General Settings')" key="general">
              <vuci-form-item-switch :uci-section="s" :label="$t('wireless.Disabled')" name="disabled"/>
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Mode')" name="hwmode" :options="radio.hwmodes" required/>
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Band')" name="htmode" :options="radio.htmodes"/>
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Channel')" name="channel" :options="radio.channels" :initial="radio.channel" required :placeholder="$t('wireless.auto')"/>
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Transmit Power')" name="txpower" :options="radio.txpowerlist" :initial="radio.txpower" required/>
            </a-tab-pane>
            <a-tab-pane :tab="$t('Advanced Settings')" key="advanced">
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Country Code')" name="country" :options="radio.countrylist" :initial="radio.country" required/>
              <vuci-form-item-input :uci-section="s" :label="$t('wireless.Distance Optimization')" name="distance" rules="uinteger"/>
            </a-tab-pane>
          </a-tabs>
        </vuci-named-section>
        <vuci-typed-section :title="$t('wireless.Interface')" type="wifi-iface" :filter="(s => s.device === radio.name)" addremove v-slot="{ s }"
                         :teasers="['disabled', 'mode', 'ssid', 'network', 'encryption']" :add="self => AddIface(self, radio.name)">
          <a-tabs>
            <a-tab-pane :tab="$t('General Settings')" key="general">
              <vuci-form-item-switch :uci-section="s" :label="$t('wireless.Disabled')" name="disabled"/>
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Mode')" name="mode" required :options="modes"/>
              <vuci-form-item-input :uci-section="s" label="SSID" name="ssid" required/>
              <vuci-form-item-select :uci-section="s" :label="$t('Network')" name="network" :options="interfaces"/>
              <vuci-form-item-switch :uci-section="s" :label="$t('wireless.Hide ESSID')" name="hidden" depend="mode == 'ap'"/>
              <vuci-form-item-switch :uci-section="s" :label="$t('wireless.WMM Mode')" name="wmm" depend="mode == 'ap'" initial/>
            </a-tab-pane>
            <a-tab-pane :tab="$t('wireless.Wireless Security')" key="security">
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Encryption')" name="encryption" :options="encryptions" initial="none" :load="loadEncr" :save="saveEncr"/>
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Cipher')" name="cipher" depend="encryption != 'none'" :options="ciphers" initial="auto" :load="loadCipher" :save="saveCipher"/>
              <vuci-form-item-input :uci-section="s" :label="$t('wireless.Passphrase')" name="key" depend="encryption != 'none'" password/>
            </a-tab-pane>
            <a-tab-pane :tab="$t('wireless.MAC-Filter')" key="mac-filter">
              <vuci-form-item-select :uci-section="s" :label="$t('wireless.Mode')" name="macfilter" :options="macfilters" depend="mode == 'ap'"/>
              <vuci-form-item-list :uci-section="s" :label="$t('wireless.MAC-List')" name="maclist" depend="macfilter == 'allow' || macfilter == 'deny'" rules="macaddr"/>
            </a-tab-pane>
          </a-tabs>
        </vuci-typed-section>
      </a-tab-pane>
