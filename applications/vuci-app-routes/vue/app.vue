<template>
  <vuci-form uci-config="network">
    <vuci-typed-section :title="$t('routes.Static IPv4 Routes')" type="route" :columns="routeColumns" addremove>
    ...
    <vuci-typed-section :title="$t('routes.Static IPv6 Routes')" type="route6" :columns="route6Columns" addremove>
      <template #interface="{ s }">
        <vuci-form-item-select :uci-section="s" name="interface" :options="interfaces" required/>
      </template>
      <template #target="{ s }">
